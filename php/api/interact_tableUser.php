<?php
    ...
    if ($_GET['action']=='get'){
        ...
        $allUser_arr = array();
        $allUser_arr['data'] = $result->fetchAll(PDO::FETCH_ASSOC);

        //convert to JSON and output
        echo json_encode($allUser_arr);
    }
    else{
        ...
        if ($_GET['action']=='add' || $_GET['action']=='edit'){
            if (!checkLen($_GET['User_name'], 5)) echo json_encode(array('status'=>'fail', 'message' => 'Username must be at least 5 characters'));
                ...
                if ($checker=='fail') echo json_encode(array('status'=>'fail', 'message' => 'This username is already taken'));
                    ...
                    if ($_GET['action']=='add'){
                        ...
                        if ($user->createUser()) echo json_encode(array('status'=>'succ'));
                        else echo json_encode(array('status'=>'fail', 'message' => 'Cannot create user'));
                    }
                    else{
                        $user->User_id = $_GET['User_id'];
                        $user->User_name = $_GET['User_name'];                        
                        $user->User_password = md5(sha1(md5($_GET['User_pass'])));
                        if ($user->update()) echo json_encode(array('status'=>'succ'));
                        else echo json_encode(array('status'=>'fail', 'message' => 'Cannot update user'));
                    }
                }
            }
        }
            ...
            else  echo json_encode(array('status'=>'fail', 'message' => 'Cannot delete user'));
        }
    }
?>